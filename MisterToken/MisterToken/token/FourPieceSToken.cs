...
namespace MisterToken {
    public class FourPieceSToken : Token {
        public FourPieceSToken(Board board, int row, int column, CellColor color1, CellColor color2, CellColor color3, CellColor color4) {
            ...
            cell2.direction = Cell.Direction.LEFT | Cell.Direction.UP;
            cell3.direction = Cell.Direction.DOWN | Cell.Direction.RIGHT;
            cell4.direction = Cell.Direction.LEFT;
            piece[0] = new TokenPiece(board, row + 1, column, cell1);
            piece[1] = new TokenPiece(board, row + 1, column + 1, cell2);
            piece[2] = new TokenPiece(board, row, column + 1, cell3);
            piece[3] = new TokenPiece(board, row, column + 2, cell4);
            orientation = 0;
        }
        ...
        public override bool RotateRight(bool dryRun, bool allowWrap) {
            ...
            switch (orientation) {
                case 0:
                    ...
                    break;
                case 1:
                    if (!piece[0].Move(dryRun, 0, 2, allowWrap)) { return false; }
                ...
                case 2:
                    ...
                    if (!piece[3].Move(dryRun, -1, 0, allowWrap)) { return false; }
                    break;
                case 3:
                    if (!piece[0].Move(dryRun, -1, -1, allowWrap)) { return false; }
                    if (!piece[2].Move(dryRun, -1, 1, allowWrap)) { return false; }
                    if (!piece[3].Move(dryRun, 0, 2, allowWrap)) { return false; }
                    break;
            }
            if (!dryRun) {
                piece[0].RotateRight();
